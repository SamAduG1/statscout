...
const MLB_MARKETS = [
  ...
  { key: 'runs',  label: 'Runs',        arr: 'runsArr',       min: 0.5, max: 1.5,  step: 0.5, defaultLine: 0.5,  playerType: 'batter' },
  { key: 'hrr',   label: 'H+R+RBI',    arr: 'hrrArr',        min: 0.5, max: 3.5,  step: 0.5, defaultLine: 1.5,  playerType: 'batter' },
  { key: 'ks',    label: 'Strikeouts',  arr: 'strikeoutsArr', min: 2.5, max: 10.5, step: 0.5, defaultLine: 5.5,  playerType: 'pitcher' },
  { key: 'outs',  label: 'Outs',        arr: 'outsArr',       min: 9.5, max: 21.5, step: 1,   defaultLine: 15.5, playerType: 'pitcher' },
];

const MLB_POS_BADGE = {
  SP: 'bg-red-500/20 text-red-400', RP: 'bg-orange-500/20 text-orange-400',
  C:  'bg-blue-500/20 text-blue-400',
  '1B': 'bg-green-500/20 text-green-400', '2B': 'bg-green-500/20 text-green-400',
  '3B': 'bg-green-500/20 text-green-400', SS: 'bg-purple-500/20 text-purple-400',
  LF: 'bg-amber-500/20 text-amber-400', CF: 'bg-amber-500/20 text-amber-400',
  RF: 'bg-amber-500/20 text-amber-400', OF: 'bg-amber-500/20 text-amber-400',
    ...
    <div className={`bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 border-l-[3px] border-l-blue-500 p-4 flex flex-col gap-3 ${onClick ? 'cursor-pointer hover:border-gray-300 dark:hover:border-gray-700 transition-colors' : ''}`}
      ...
      <div>
        <div className="flex items-baseline gap-1.5">
          ...
          <span className="text-xs text-gray-400">O {line}</span>
        </div>
        <div className="h-1.5 bg-gray-100 dark:bg-gray-800 rounded-full mt-1.5 overflow-hidden">
          <div className={`h-full rounded-full transition-all ${hitRate >= 70 ? 'bg-green-500' : hitRate >= 55 ? 'bg-blue-500' : 'bg-red-400'}`}
            style={{ width: `${hitRate ?? 0}%` }} />
...
export default function StatScoutDashboard() {
  ...
  const mlbView = currentSport === 'mlb' ? (pathParts[1] === 'players' ? 'playerProps' : 'gameTotals') : 'gameTotals';
...
const handleLineAdjust = (playerId, playerName, statType, newData) => {
  ...
  // Fetch ALL soccer players for a league when entering player props view
  // (No fixture required — works like NBA/MLB all-players view)
  useEffect(() => {
    if (currentSport !== 'soccer' || soccerView !== 'playerProps') return;
    const cacheKey = `__all__${soccerLeague}`;
    if (soccerPlayersData[cacheKey]?.players || soccerPlayersData[cacheKey]?.loading) return;
    setSoccerPlayersData(prev => ({ ...prev, [cacheKey]: { loading: true, error: null } }));
    let retryTimeout = null;
    const fetchAll = async (attempt = 1) => {
      try {
        const res = await fetch(`${API_BASE_URL}/soccer/players/all?league=${soccerLeague}`);
        const data = await res.json();
        if (data.success && data.count > 0) {
          setSoccerPlayersData(prev => ({ ...prev, [cacheKey]: { players: data.players, loading: false, error: null } }));
        } else if (attempt < 12) {
          retryTimeout = setTimeout(() => fetchAll(attempt + 1), 5000);
        } else {
          setSoccerPlayersData(prev => ({ ...prev, [cacheKey]: { players: [], loading: false, error: 'No player data found. Run populate_soccer_players.py first.' } }));
        }
      } catch {
        setSoccerPlayersData(prev => ({ ...prev, [cacheKey]: { players: [], loading: false, error: 'Failed to load player data.' } }));
      }
    };
    fetchAll();
    return () => { if (retryTimeout) clearTimeout(retryTimeout); };
  }, [currentSport, soccerView, soccerLeague]);
        ...
        <div className={`max-w-7xl mx-auto px-6 ${currentSport !== 'mlb' ? 'py-8' : ''}`}>
          ...
          {currentSport === 'soccer' && (
            <div>
              ...
              {/* PLAYER PROPS VIEW — all players, no fixture required */}
              {soccerView === 'playerProps' && (() => {
                const cacheKey = `__all__${soccerLeague}`;
                const state = soccerPlayersData[cacheKey];
                const allPlayers = state?.players || [];
                const isLoading = !state || state.loading;
                const isError = state && !state.loading && state.error;
                ...
                return (
                  <>
                      ...
                      {SOCCER_MARKETS.map(m => (
                        <button key={m.key} onClick={() => setSoccerPropsMarket(m.key)}
                          className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                            soccerPropsMarket === m.key ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-gray-200'
                          }`}>
                          {m.label}
                        </button>
                      ))}
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-3">
                      ...
                      {!isGk && (
                        <div className="flex gap-1">
                          {[null, 'FW', 'MF', 'DF'].map(pos => (
                            <button key={String(pos)} onClick={() => setSoccerPropsPosition(pos)}
                              className={`px-2.5 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                                soccerPropsPosition === pos ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-gray-200'
                              }`}>
                              {pos ?? 'All'}
                            </button>
                          ))}
